...
public class FineTuningClient {
    ...
    public String generate(String prompt) {
        ...
        List<FineTuneResponse> responses = webClient.post()
                ...
                .onErrorResume(FineTuneServiceException.class, Mono::error) // Không wrap lại nếu đã là FineTuneServiceException
                .onErrorResume(e -> {
                    log.error("HF Inference API that bai sau khi retry: {}", e.getMessage());
                    return Mono.error(new FineTuneServiceException(
                            "Mô hình Fine-tuning hiện không phản hồi. Vui lòng thử lại sau vài giây.", e));
                })
                .block();
